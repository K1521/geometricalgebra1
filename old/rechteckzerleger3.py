...
while imagemap.max()!=0:
    ...
    bestrects=[[] for _ in range(num_features)]
    ...
    for y in range(imagemap.shape[0]):
        ...
        for x,down in enumerate(chain(distancematrix[y],[0])):
            ...
            while considering and considering[-1][1]>=down:
                ...
                rectset.append((arealast,lastx,y,(x-lastx),lastdown))
    ...
    imagemapc=imagemap.copy()
    for bestset in bestrects:
        bestset.sort(reverse=1)
        for bestrect in bestset:
            area,x,y,w,h=bestrect


            #if not imagemap[[y,y,y+h-1,y+h-1],[x,x+w-1,x,x+w-1]].all():
            if not imagemapc[y:y+h,x:x+w].all():
            #if 0 in imagemapc[y:y+h,x:x+w].ravel():
                imagemapc[y:y+h,x:x+w]=0
                continue
            #if not imagemapc[y:y+h,x:x+w].all():
            #    continue
            imagemapc[y:y+h,x:x+w]=0
