...
plotter.add_mesh(cube_mesh,opacity=0.5)
...
def apply_force(rigid_body, point, force, dt):
    torque = np.cross(rigid_body.rotation_matrix@point, force)
    rigid_body.velocity += (force / rigid_body.mass) * dt
    rigid_body.angular_momentum += torque * dt
    arrow.shallow_copy(pv.Arrow(start=rigid_body.position,direction=torque/10,scale='auto'))
...
def apply_spring_force(rigid_body, point, spring_constant, dt):
    vertex_position = (rigid_body.rotation_matrix@point) + rigid_body.position
    force = calculate_spring_force(vertex_position, spring_constant)
    apply_force(rigid_body, point, force, dt)
...
while True:
    ...
    for i in range (10):
        apply_spring_force(cube, cube_points[0], spring_constant, dt)
        ...
        cube.rotation_matrix=cube.rotation_matrix*1.5-0.5*cube.rotation_matrix@cube.rotation_matrix.T@cube.rotation_matrix
