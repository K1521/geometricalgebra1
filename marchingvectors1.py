...
xtf,ytf,ztf=(xyzderiv(var,d)for var,d in zip((x,y,z),[[1,0,0],[0,1,0],[0,0,1]]))
iprod=point(xtf,ytf,ztf).inner(vis)
voltf=sum(abs(blade.magnitude) for blade in iprod.lst)
vol=voltf.f
delta=np.stack(voltf.df,axis=3)
print("calculated")
...
with tf.GradientTape(persistent=True) as tape:
    ...
    voltf=sum(tf.abs(blade.magnitude) for blade in iprod.lst)
    vol=voltf#iprod.toscalar()#voltf
...
arrows=[p.add_arrows(punkte[i,:,:,:], delta[i,:,:,:], mag=0.1,show_scalar_bar=False) for i in range(step)]
for a in arrows:
    a.visibility=False
alast=arrows[0]
def slideraction(v):
    ...
    if a!=alast:
        ...
        alast=a
p.add_slider_widget(slideraction, [0, step-1])

#p.add_arrows(punkte[step//2,:,:,:], delta[step//2,:,:,:], mag=0.1,show_scalar_bar=False)
