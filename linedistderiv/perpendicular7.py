...
def evalsquare(festeaxis,offset):
    ...
    spi=np.sum(pi,axis=0)
    snmi=np.sum(nmi,axis=0)
    A=np.sum(pi**2)+spi.dot(spi)/n
    #A=sum(pi**2)+sum(pi)**2/n
    B=np.sum(pi*ni)-spi.dot(np.sum(ni,axis=0))/n
    #B=sum(pi*ni)-sum(pi)*sum(nmi+ni)/n
    #B=sum(pi*ni)-sum(pi)*sum(ni)/n
    C=np.sum(ni**2)+snmi.dot(snmi)/n-2*np.sum(nmi,axis=0).dot(np.sum(ni,axis=0))/n
    #C=sum(ni**2)+sum(nmi)**2/n-2*sum(nmi*ni)/n
    #print(A,B,C)
    a=-B/C
    avgpoint=spi/n+a*snmi/n
    e=A-B**2/C
    #print(avgpoint)
